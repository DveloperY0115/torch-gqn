...
parser.add_argument("--gen_interval",
                    type=int,
                    default=100,
...
parser.add_argument("--save_interval",
                    type=int,
                    default=10000,
                    help="Period for making checkpoint")
...
def train_one_epoch(
    ...
    for i, (f_batch, c_batch) in enumerate(train_dataloader):
        ...
        sigma_t = max(
            args.sigma_f + (args.sigma_i - args.sigma_f) *
            (1 - i / args.sigma_n),
            args.sigma_f,
        )
...
def main():
    ...
    # construct model
    model = GQNCls(repr_architecture="Tower",
                   L=args.level,
    ...
    for s in tqdm(range(s_begin, int(args.max_step))):
        ...
        sigma_t = max(
            args.sigma_f + (args.sigma_i - args.sigma_f) * (1 - s / (2e5)),
            args.sigma_f)
